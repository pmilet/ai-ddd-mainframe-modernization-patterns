...
A second epistemic note. The catalog is not just refining the patterns. It is shaping the language itself. Three fields the book draws on each have their own vocabulary, and there is no settled term for many of the things this catalog needs to name — what is the right word for the kind of substrate that holds vocabulary? What do we call the joint discovery of slices and seams? When does an anti-corruption layer become a transitional published language? The intersection of mainframe modernization, AI engineering, and contemporary Domain-Driven Design is new enough territory that the words for it are still being chosen. This catalog is one attempt at choosing them. Some terms here will turn out to be the right names and persist; others will be replaced by better ones the field converges on. That work — the shaping of the vocabulary — is part of what the catalog is for, and it is unfinished by design.
...
**Recovery.** Recovery from the legacy is partial. Vocabulary inference from comments, display literals, naming conventions, the intermediate representation, and the data layer’s DDL — DB2 schemas, VSAM definitions, DCLGEN copybooks — provides candidate terms. The data layer often preserves domain vocabulary better than the procedural code does: DDL columns frequently retain canonical business terms that working-storage variables in COBOL paragraphs have abbreviated, prefixed, or renamed for technical convenience. Semantic similarity over code units surfaces clusters that may correspond to ontological concepts. Operational artifacts — scheduler dependencies, transaction names, dataset catalogs — carry vocabulary the source code may have lost. These are starting points, not conclusions. Workshop techniques like Event Storming (Brandolini) accelerate the human side — domain experts, developers, and operators in a room mapping events, commands, and aggregates against shared vocabulary. The architect or domain expert validates each candidate, refines vocabulary, reconciles drift, and articulates the canonical concepts the modernized system will encode.

**What the substrate enables.** Once established, the ontology is the artifact a number of activities operate against. *Knowledge crunching* — Evans’ term for the iterative work of moving from naive descriptions of the domain to deeper, sharper models — has a destination: refinements land here, not in someone’s notebook. *Disambiguation* — when two stakeholders disagree about what *active customer* means, the ontology is the artifact they reach for; it either resolves the disagreement by surfacing the canonical definition or makes the disagreement legible enough that the ontology itself needs revision. *Boundary evidence* — points where a word changes meaning across the legacy are evidence of bounded-context boundaries the modernization will need to honour; the vocabulary substrate carries these as one of several inputs to the decomposition activity (alongside the capability map of Pattern 1, the structural clusters of Pattern 3, and the slice signals of Pattern 5). *Debt management* — the substrate makes both technical and cognitive debt visible, sized, and prioritisable. *Technical debt* is the cost of code that no longer matches the architecture the team would design today. *Cognitive debt* is the cost of vocabulary that no longer matches the concepts the team would name today. It doesn’t pay either debt down; it makes each discrepancy nameable, so the team can decide what to carry, what to refactor, and what to defer to the bridge period (Pattern 27) where the migration translates between legacy and canonical at the boundary.
...
This pattern has not yet been built into Rosetta as a first-class substrate. The principle is articulated; the substrates that feed recovery (vocabulary inference, semantic clustering, operational artifacts) are operational; what remains is the ontology substrate itself, as a first-class artifact with the operational machinery to validate, refine, and maintain canonical vocabulary independently of the structural and semantic substrates. The work is in design; the foundation it builds on is in place.
...
A legacy codebase parsed as a graph and indexed for semantic similarity (Pattern 3). The graph contains thousands of paragraphs, hundreds of programs, dozens of [bounded contexts](#gl-bounded-context). The modernization needs to identify two related but distinct things from this substrate: **[vertical slices](#gl-vertical-slice)** — coherent feature units that can be extracted, scaffolded, translated, and verified as working wholes — and **seams** — technical points where the modernization can intervene in the legacy without rewriting it.

Capability ⊃ bounded context ⊃ vertical slice ⊃ aggregate. The catalog uses these as nested concepts of different scope: a capability spans contexts, a bounded context contains many slices, a slice touches one or more aggregates within a context. Each level is the unit of a different kind of work. Slice discovery is *tactical* work at the granularity below bounded context and above paragraph.
...
Good seams have characteristic properties (Ferri & Coggrave 2024): **observable** — the interface produces evidence about what passed through it; **divertable** — traffic at the seam can be routed elsewhere without breaking what depends on it; **usable externally** — code outside the legacy can interact with the seam (HTTP, file transfer, message queue, CDC stream); and **funnel-like** — the surface area is narrow enough that the team can reason about it. A seam candidate that fails any of the four is not a working seam, however structurally clean it appears.
...
In DDD vocabulary, the artifact that articulates these relationships is the **[context map](#gl-context-map)**. Eric Evans introduced the concept in *Domain-Driven Design* (2003) and named seven characteristic relationship types between bounded contexts: [shared kernel](#gl-shared-kernel), [customer-supplier](#gl-customer-supplier), [conformist](#gl-conformist), [anti-corruption layer](#gl-anti-corruption-layer), [separate ways](#gl-separate-ways), [open host service](#gl-open-host-service), and published language. Vaughn Vernon elaborated their operational mechanics in *Implementing Domain-Driven Design* (2013). Nick Tune and Krisztina Hirth’s *Bounded Context Canvas* (DDD Crew, ongoing) is the most actively-maintained contemporary articulation — *most actively-maintained* meaning it is still revised in public, with an open-source canvas and worked examples updated as the practice evolves rather than frozen at publication. For a modernization team, that currency matters: it reflects how context mapping is done today, not only how it was first defined.

This catalog adapts the context map for the specific shape of modernization work. The adaptation is *structural, not cosmetic* — it changes what the artifact is and does, not how it is drawn. A cosmetic adaptation would restyle Evans’ context map for mainframes; a structural one changes its shape. The modernization context map is *temporal* (relationships change as contexts transition through stages), *stacked* (each strategic decision constrains the next), and *populated by three kinds of bounded contexts simultaneously* — modernized business contexts, agentic-platform contexts, and external-vocabulary contexts. The Pattern section develops each property; the consequence of taking them together is the point worth stating up front: the context map becomes the place where the migration plan is *derivable* from strategic decisions rather than improvised under operational pressure once the modernization is already underway.
...
A final force complicates the picture in AI-assisted modernization. The bounded contexts on the map are not only the modernized business contexts. The agentic platform has its own (Pattern 19’s MCP servers, each with its own [ubiquitous language](#gl-ubiquitous-language) and consistency boundary; the LLMs themselves, probabilistic, with their own natural-language interface contract). The external vocabularies the system integrates with (industry taxonomies, regulatory standards) are bounded contexts too. **A context map that omits any of these three populations is incomplete** — in a way that produces real failures, not just an untidy diagram. Omit the agentic-platform contexts and the deterministic/probabilistic boundary goes ungoverned, so model vocabulary leaks into the domain (the failure Pattern 19’s ACL prevents). Omit the external-vocabulary contexts and integrations with industry standards are designed ad hoc, each team reinventing a mapping to NAICS or ACORD that should have been one shared published-language relationship. A map showing only the business contexts hides two-thirds of the surface the modernization has to govern.
...
**Bounded-context boundaries are recovered from multiple signals, side-effect clustering among them.** The standard DDD test for a bounded context is linguistic: a context is where a single ubiquitous language applies. In a mainframe whose language has drifted, that test alone is partial. A complementary signal carries the same information: slices that share side-effects belong together; slices that don't, don't. Two slices writing the same tables, dispatching onto the same queues, or calling the same external systems are operationally coupled regardless of shared vocabulary; two that share vocabulary but no side-effects are operationally independent. The side-effect graph from Pattern 5 is therefore boundary evidence in its own right, complementary to the vocabulary-boundary evidence from Pattern 4: a candidate context whose internal slices share side-effects mostly with each other is structurally plausible; one whose slices are tangled with slices outside it is probably the wrong boundary. The linguistic test can itself be made an agentic verification assessment — an agent checking a candidate boundary’s vocabulary against the ontology and surfacing where terms stay coherent, shift at the edge (confirming the boundary), or shift within it (contradicting it). That assessment is developed in Pattern 15; the context map consumes its result as one more boundary signal.

**Three populations of bounded contexts participate.** The context map for an AI-assisted mainframe modernization covers three distinct populations simultaneously:

- **Modernized business contexts** — what most context maps cover. Claims, Underwriting, Billing, Pricing, Customer Master, and whatever else the legacy decomposed into. The transitional-vs-durable distinction is sharpest here: most relationships *between* legacy and modernized versions of the same business context are transitional and will retire at cutover.
- **Agentic-platform contexts** — Pattern 19’s bounded MCP servers, each with its own ubiquitous language and consistency boundary. The LLMs themselves are also bounded contexts: as Evans articulates in *Context Mapping with an AI-based Component* (2026), an LLM has its own language (natural-language prompts), its own consistency model (probabilistic), and its own interface contract. The layer between the deterministic application and the probabilistic LLM is an anti-corruption layer in the canonical Evans sense. Relationships in this population are mostly durable — they will hold for the lifetime of the agentic platform, evolving as the platform does but not retiring with the modernization.
- **External-vocabulary contexts** — industry classification systems (NAICS, BIAN, FIBO, ACORD, GS1), regulatory taxonomies, ontology sources Pattern 4 draws on. These are bounded contexts the modernized system enters into *Published Language* relationships with. Most are durable; their relationship type rarely changes once established.

Each population has its own relationship-type mix and its own typical lifetime profile, but the discipline of naming relationships and their retirement criteria is the same across all three.

**Cross-context seams are also published-language enforcement points.** Pattern 5 names the technical seams at boundaries; Pattern 6 names the strategic relationship those seams carry; the two views compose. A seam that crosses a bounded-context boundary is not just an intervention point — it is the place where the published language between those contexts is operationally enforced. The four good-seam criteria from Pattern 5 (observable, divertable, externally usable, funnel-like) are simultaneously the criteria for a usable published-language interchange: a seam that fails any of them cannot carry a published language reliably. Within-context seams (instrumentation, traffic mirroring, internal divert) carry traffic but not semantics; cross-context seams carry both, and the published language is what gives the semantic crossing its integrity. The context map names which relationships carry published language; the seam typology names the points at which those published languages cross.

**Engage all of Evans' relationship types** — across all three populations:

- **Shared kernel** for contexts that genuinely share a small, stable core (e.g., a shared *Money* type across financial business contexts; a shared *Reasoning Record* schema across agentic-platform contexts). Requires explicit team-level agreement on what is in the kernel and what is not.
- **Customer-supplier** where one context's needs influence another's design. The modernized side as customer of a still-legacy supplier during dual-run is the canonical modernization case; the agentic platform as customer of a model provider is the canonical AI-engineering case. The supplier explicitly negotiates its outputs to meet the customer's needs.
- **Conformist** where one side has no leverage over the upstream and the upstream's vocabulary is acceptable. Rare in well-designed modernization — usually a sign that an ACL is missing.
- **Anti-corruption layer** where one side cannot accept the upstream's vocabulary and cannot influence it either. *Transitional* by default in modernization contexts (retirement criterion: the legacy is retired); *durable* in agentic-platform contexts (the deterministic-probabilistic boundary is permanent).
- **Separate ways** where two contexts have no integration that earns its keep. A legitimate relationship; the catalog should not pressure integration where none is justified.
- **Open host service** where a context publishes one well-documented, stable interface for general use, so consumers adopt it as-is rather than each negotiating a bespoke integration (defined in the Glossary). The modernized side often publishes open host services to replace undocumented point-to-point mainframe integration the legacy never articulated as services.
- **Published language** where two contexts integrate through a deliberately stable, publicly-documented vocabulary. The canonical ontology (Pattern 4) anchors published languages between business contexts; commands and events (Pattern 11) implement them at runtime; industry taxonomies are the canonical examples in external-vocabulary contexts. This three-way correspondence — ontology defines the vocabulary, commands/events carry it, taxonomies exemplify it — is something agents can continuously police: a standing structure-and-ontology matching check that compares the vocabulary each context actually emits at its boundaries against the published language it is supposed to honour, flagging drift as risk and unrecognised shared vocabulary as opportunity. That continuous check is developed in Pattern 22 (the harness as standing observer), with Pattern 19’s bounded MCP servers as where the enforcement bites at the boundary.

### Consequences

Cross-context relationships become first-class architectural decisions, named and defended in review across all three populations. The transition has a strategic spine, not just an operational one. When a relationship goes wrong in production, the team can trace the failure back to a specific design decision rather than to accumulated drift.

Transitional relationships have explicit retirement criteria. Anti-corruption layers, bridge APIs, customer-supplier relationships with not-yet-modernized legacy modules — each carries a named condition for its own removal. The infrastructure built for transition is designed for retirement, not for permanent operation. Pattern 27 operationalises the lifecycle the context map names strategically.
...
The agentic platform and the modernization share one strategic register. By treating MCP servers, LLMs, and external vocabularies as participants in the same context map, the discipline of relationship-typing and retirement-criterion-naming applies uniformly. There is no separate strategic artifact for the AI platform; it lives in the same map as the business contexts.
...
The context map makes technical debt and transitional architecture manageable over time. Each transitional relationship is a piece of named, scoped, time-bounded debt — an ACL, a bridge API, a conformist accommodation — recorded with the condition for its repayment. Instead of transitional architecture being an undifferentiated mass of "temporary" infrastructure that quietly becomes permanent, it is an itemised ledger: this relationship is transitional and retires when the legacy Claims module is decommissioned; that one is durable and stays. The map turns the transition from a fog into a backlog. A team can look at the context map at any point in the modernization and answer the two questions that transitional architecture usually cannot: *how much of what we have built is temporary, and what has to become true for each piece of it to go away?*
...
This pattern has not yet been built into the Rosetta prototype as a versionable artifact; the principle is articulated and the infrastructure to capture context maps as queryable substrates alongside capability map and ontology is in design. Like Pattern 4, this pattern stands on Evans’ typology plus the catalog’s framing of strategic artifacts as queryable substrates; the operational machinery is the part still in design.

### Related patterns

Pattern 1 (*Business-Aligned Capability Strategy*) determines which capabilities exist and how they are treated; subdomain types from Pattern 1 constrain which relationship types are coherent in this pattern's map, and the capability strategy sets the sequencing priority for which transitional relationships are retired first. Pattern 4 (*Domain Ontology as Independent Substrate*) anchors the canonical vocabulary that published-language relationships use. Pattern 5 (*Slice and Seam Discovery*) is the technical counterpart at the boundary altitude — where the context map names *what kind of relationship* a context pair has, Pattern 5 names *where to operate* on that relationship; cross-context seams are also the published-language enforcement points the context map identifies. Pattern 11 (*Commands and Events as Logical Boundary*) is the implementation mechanism for published-language relationships specifically. Pattern 19 (*Bounded MCP Servers*) provides the agentic-platform contexts that participate in the context map alongside business contexts. Pattern 24 (*Team Topology and Bounded Context Alignment*) is the organisational counterpart — customer-supplier relationships in the context map should align with team relationships in the topology, and where they do not, the misalignment is itself a design concern. Pattern 27 (*Dual-Run Coexistence*) is where the transitional-relationship discipline is operationally exercised; bridge APIs are anti-corruption layers in the context-map sense, and the ACL lifecycle Pattern 27 develops realises operationally what this pattern names strategically. Pattern 28 (*Replatform with Modern Facade*) is the operational pattern for relationships the context map types as open host service to an external consumer.

Eric Evans’ original treatment (*Domain-Driven Design*, 2003), Vaughn Vernon’s operational elaboration (*Implementing Domain-Driven Design*, 2013), Nick Tune and Krisztina Hirth’s *Bounded Context Canvas* (DDD Crew, ongoing), and Evans' *Context Mapping with an AI-based Component* (2026) are the lineage this pattern stands on. What this catalog contributes is the *stacked* framing — subdomain identity constrains coherent relationship types, relationship types constrain which carry published language, published-language relationships are enforced at cross-context seams, and each relationship type implies a specific operational pattern — composed into a single temporally-versioned artifact that spans three populations of bounded contexts and is readable as a migration plan.
...
The patterns in this group address [tactical design](#gl-tactical-design) as DDD uses the term: how each bounded context materialises in modern code. Aggregates, [domain events](#gl-domain-event), handlers, the architecture chosen for each context. They presuppose strategic recovery from Part I — without bounded contexts identified and ubiquitous language established, generation produces structurally plausible code that doesn’t reflect the domain.
...
An agentic system for software engineering, where LLMs participate in code generation. The system has both deterministic decisions (what scaffold to render, what [bounded contexts](#gl-bounded-context) exist, what types to declare) and probabilistic decisions (how to translate idiomatic COBOL into idiomatic C#, how to handle edge cases, how to phrase things). How these two kinds of work are organised determines whether the system is reproducible and auditable, or opaque and unreliable.
...
A pipeline that has both a flexible substrate where analysis happens (the graph, where [bounded contexts](#gl-bounded-context) emerge and uncertainty lives) and a strict surface where generation happens (the emitter, where C# is produced through Roslyn). The two need to communicate. Naively connecting them couples the analysis to the generation in ways that resist evolution.
...
A modernization pipeline producing C# scaffolds for many [bounded contexts](#gl-bounded-context) within a single legacy system. Each bounded context has different domain weight — some are core to the business and likely to evolve significantly, others are generic supporting logic that should be cheap to maintain.

### Problem

Most modernization platforms apply a single architectural template across the entire codebase. The result is that generic domains get over-engineered ([hexagonal architecture](#gl-hexagonal-architecture) for a lookup table is masochism) and core domains get under-engineered ([vertical slices](#gl-vertical-slice) for a pricing engine collapse under their own weight in a few years).
...
- **Tier 3** (strategic core, the crown jewels): full hexagonal architecture. Domain at the centre, ports and adapters, the structure that pays its tax over the system’s lifetime. This is where Evans’ insistence on protecting the [core domain](#gl-core-domain) from infrastructural concerns earns its keep.
...
A modernization pipeline whose target architecture is one of multiple possible architectures. The choice of target depends on the engagement — some teams want [vertical slice](#gl-vertical-slice) architecture (VSA) with Wolverine handlers, others want [hexagonal architecture](#gl-hexagonal-architecture) with explicit ports and adapters, others want a Java target instead of C#, others may want a hybrid approach for tier-2 [bounded contexts](#gl-bounded-context) that balances VSA velocity with hexagonal protection. The target may evolve as understanding of the legacy improves, or as the team’s priorities shift across the lifetime of the modernization.

The same pipeline must also serve constituencies that consume something other than code. Architects reviewing [strategic design](#gl-strategic-design) need C4 model views. Data engineers validating ER models need entity-relationship diagrams. DDD practitioners checking aggregate boundaries need aggregate maps and [context maps](#gl-context-map). Business analysts reading workflows need data flow diagrams. Operators tracing state need event maps. Regulators auditing decisions need architecture decision records. Each constituency wants a different view of the same architecture.
...
- **Context maps** (DDD-specific, Evans 2003) — rendered from bounded-context relationships in the graph and command/event flows in the IR; shows upstream/downstream relationships, [anti-corruption layers](#gl-anti-corruption-layer), [shared kernels](#gl-shared-kernel), integration patterns.
...
A modernized system structured as a set of [bounded contexts](#gl-bounded-context), each generated through the patterns above. The bounded contexts must communicate with each other to implement business workflows that span them. The communication may need to be synchronous in-process initially (for performance, for transactional simplicity, for deployment minimalism) but may need to evolve toward asynchronous messaging or distributed services later (for scaling, for team boundary alignment, for operational independence).
...
Where the modernized system must coexist with parts of the legacy that are not yet migrated, an **[anti-corruption layer](#gl-anti-corruption-layer)** intercepts the legacy interaction and translates between the legacy’s vocabulary and the canonical [ubiquitous language](#gl-ubiquitous-language) of the modernized side. The anti-corruption layer is Eric Evans’ term for a translation layer that prevents legacy concepts from polluting the modernized domain model. It is itself a bounded context with a single responsibility — protect the modernized domain from inheriting whatever the legacy still calls things.
...
- **Documentation re-renders** — the C4 model views, aggregate maps, [context maps](#gl-context-map), ER diagrams (Pattern 10) regenerated whenever the substrates change. Reviewers see the architectural shape, not just the code shape.
...
- Current code emitters: [vertical slice](#gl-vertical-slice) (Wolverine, C#), hexagonal (Wolverine with ports/adapters, C#), hybrid (Wolverine with lightweight domain models, C#)
...
- Eric Evans, *Domain-Driven Design: Tackling Complexity in the Heart of Software* (Addison-Wesley, 2003). The founding text. Strategic design, bounded contexts, the seven context-map relationship types, [ubiquitous language](#gl-ubiquitous-language), aggregates. Patterns 4, 6, 11, and the strategic spine of the catalog rest on it.
...
- Eric Evans, *Context Mapping with an AI-based Component* (2026). The source for the LLM-as-bounded-context framing developed in Patterns 6 and 19.
...
**The first claim is that mainframe modernization is, at its core, a Domain-Driven Design activity at scale.** Strategic design — recovering the domain, identifying bounded contexts, establishing ubiquitous language — is the work that determines whether the modernization delivers business value. Tactical design — aggregates, [domain events](#gl-domain-event), handlers — is the work that determines whether the modernized code is maintainable. The patterns in Parts I and II operationalise both. If this claim is right, modernizations that skip the DDD work will produce technically successful systems that fail commercially. If it’s wrong, modernizations can succeed through behavioural fidelity alone.

**The second claim is that AI assistance changes the economics of modernization but not its discipline.** What was previously infeasible at scale — recovering specifications from decade-old COBOL, generating idiomatic C# at paragraph granularity, verifying behavioural equivalence in tight inner loops — becomes tractable when agents handle the mechanical work. But the discipline that determines whether the modernization succeeds is the same as it was before agents existed: [strategic design](#gl-strategic-design) done well, [tactical design](#gl-tactical-design) grounded in canonical ontology, verification that grades against ground truth rather than against the team’s own assumptions. The patterns in Parts III and IV are the discipline; the agents are the labour. If this claim is right, teams investing in agents without investing in discipline will produce faster modernizations that fail at the same rate as manual ones. If it’s wrong, the agents themselves are enough.
