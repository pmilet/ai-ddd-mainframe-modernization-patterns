...
**Bounded-context boundaries are recovered from multiple signals, side-effect clustering among them.** The standard DDD test is linguistic: a bounded context is where a single ubiquitous language applies. In a mainframe whose language has drifted, that test alone is partial. A second signal carries the same boundary information: **slices that share side-effects belong together; slices that don't, don't.** Two slices writing to the same tables, dispatching onto the same queues, calling the same external systems are operationally coupled regardless of whether they share vocabulary. Conversely, two slices that share vocabulary but no side-effects are *operationally independent* — the same words being used in two contexts that don't actually talk to each other. The side-effect graph from Pattern 5 is therefore boundary evidence in its own right, complementary to the vocabulary-boundary evidence from Pattern 4. A candidate bounded context whose internal slices share side-effects with each other and only minimally with slices outside is structurally plausible; one whose slices are operationally tangled with slices outside the candidate is probably the wrong boundary.
...
**Semantic index.** The catalog’s term for the proximity-queryable substrate that complements the structural graph (Pattern 3). In implementation, a vector database holding embeddings of code chunks, comments, documentation, IR slots, and ontology terms — queryable through approximate-nearest-neighbour search. The semantic index answers *similarity* questions (“what code is semantically like this?”) where the graph answers *structural* questions (“what calls this paragraph?”). The two substrates have different epistemologies: the graph is discrete and exact, the index is continuous and approximate. *Related to **Embedding** and **Vector database**; central to Pattern 3.*
